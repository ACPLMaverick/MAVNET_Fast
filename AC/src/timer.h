...
#define Timer_MsFromTickCount(ticks)            ((uint16_t)(( (float)(ticks) / (float)F_CPU) * 1000.0f))
#define Timer_UsFromTickCount(ticks)            ((uint32_t)(( (float)(ticks) / (float)F_CPU) * 1000000.0f))
...
#define Timer_ScheduleCallback_1(ticks, func, param, callNum) Timer_ScheduleCallback_1_Ext(Timer_OverflowNumFromTicks_16(ticks), Timer_RemainderFromTicks_16(ticks), func, param, callNum)
...
#define Timer_ScheduleCallback_2(ticks, func, param, callNum) Timer_ScheduleCallback_2_Ext(Timer_OverflowNumFromTicks_8(ticks), Timer_RemainderFromTicks_8(ticks), func, param, callNum)
...
inline uint16_t Timer_GetDurationMs(void) { return Timer_MsFromTickCount(Timer_GetDurationTicks()); }
inline uint32_t Timer_GetDurationUs(void) { return Timer_UsFromTickCount(Timer_GetDurationTicks()); }
