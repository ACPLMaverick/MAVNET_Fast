...
namespace je { namespace draw { namespace gpu {
    ...
    bool pso_vulkan::init(device& a_device, const pso_params& a_params)
    {
        ...
        const math::screen_size wanted_viewport_size = compute_screen_size(a_params.m_source_screen_size, m_state.m_viewport_perc_offset);
        const math::screen_size wanted_scissor_size = compute_screen_size(a_params.m_source_screen_size, m_state.m_scissor_perc_offset);
        VkViewport viewport{};
        viewport.x = static_cast<float>(m_state.m_viewport_perc_offset.z);
        viewport.y = static_cast<float>(m_state.m_viewport_perc_offset.w);
        viewport.width = static_cast<float>(wanted_viewport_size.x);
        viewport.height = static_cast<float>(wanted_viewport_size.y);
        viewport.minDepth = 0.0f;
        viewport.maxDepth = 1.0f;
        VkRect2D scissor{};
        scissor.offset.x = m_state.m_scissor_perc_offset.z;
        scissor.offset.y = m_state.m_scissor_perc_offset.w;
        ...
        rasteriser_info.depthClampEnable = m_state.m_shadow_depth_bias_and_clamp;
        rasteriser_info.rasterizerDiscardEnable = VK_FALSE;
        rasteriser_info.polygonMode = VK_POLYGON_MODE_FILL;
        rasteriser_info.cullMode = m_state.m_cull_backface ? VK_CULL_MODE_BACK_BIT : VK_CULL_MODE_NONE;
        rasteriser_info.frontFace = VK_FRONT_FACE_CLOCKWISE;
        rasteriser_info.depthBiasEnable = m_state.m_shadow_depth_bias_and_clamp;
        rasteriser_info.depthBiasConstantFactor = m_state.m_shadow_depth_bias_and_clamp ? 0.0f : 0.0f; // TODO
        rasteriser_info.depthBiasClamp = m_state.m_shadow_depth_bias_and_clamp ? 0.0f : 0.0f;   // TODO
        rasteriser_info.depthBiasSlopeFactor = m_state.m_shadow_depth_bias_and_clamp ? 0.0f : 0.0f; // TODO
        ...
        ds_info.depthTestEnable = m_state.m_depth_test_mode != depth_test_mode::k_none;
        ds_info.depthWriteEnable = m_state.m_depth_write;
        switch(m_state.m_depth_test_mode)
        {
        ...
        ds_info.stencilTestEnable = m_state.m_stencil_mode != stencil_mode::k_none;
        switch (m_state.m_stencil_mode)
        {
        case stencil_mode::k_write:
            ...
            ds_info.front.writeMask = m_state.m_stencil_value;
        ...
        case stencil_mode::k_test:
            ...
            ds_info.front.compareMask = m_state.m_stencil_value;
        ...
        blend_attachment.blendEnable = m_state.m_blend_mode != blend_mode::k_overwrite;
        switch(m_state.m_blend_mode)
        {
