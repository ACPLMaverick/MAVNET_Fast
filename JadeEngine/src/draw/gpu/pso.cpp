...
namespace je { namespace draw { namespace gpu {
    ...
    {
        return m_shaders[static_cast<size>(a_stage)] != nullptr;
    }

    bool render_state::has_valid_push_constants_buffer() const
    {
        return m_push_constants_buffer.size() > 0;
    }

    bool render_state::has_valid_viewport() const
    {
        return m_viewport_perc_offset.x > 0
            && m_viewport_perc_offset.y > 0;
    }

    bool render_state::has_valid_scissor() const
    {
        return m_scissor_perc_offset.x > 0
            && m_scissor_perc_offset.y > 0;
    }

    bool render_state::has_valid_multisampling() const
    {
        return m_multisample_count != multisample_count::k_none
            && m_multisample_mode != multisample_mode_flag::k_disabled;
    }

    pso::pso(const pso_params& a_params)
        : m_state(a_params.m_render_state)
        , m_hash(m_state)   // TODO Properly compute hash from shaders and data buffers!
    {
    }
